...
# q_n = lasagne.layers.ExpressionLayer(q_n, lambda x: T.exp(x), output_shape='auto')
# q_n = lasagne.layers.ExpressionLayer(q_n, lambda x: x / x.norm(2, axis=1).dimshuffle((0, 'x', 1)), output_shape='auto')
...
# a_n = lasagne.layers.ExpressionLayer(a_n, lambda x: T.exp(x), output_shape='auto')
# a_n = lasagne.layers.ExpressionLayer(a_n, lambda x: x / x.norm(2, axis=1).dimshuffle((0, 'x', 1)), output_shape='auto')
...
cost = T.maximum(0, (MARGIN - correct_cos_sim/2 + wrong_cos_sim/2)).mean() + cost_l2
